...
def fc_layer(inputs, hiddens, idx, nonlinearity=concat_elu, flat = False):
  with tf.variable_scope('{0}_fc'.format(idx)) as scope:
    ...
    weights = _variable('weights', shape=[dim,hiddens],initializer=tf.truncated_normal_initializer(stddev=0.5))
    biases = _variable('biases', [hiddens], initializer=tf.truncated_normal_initializer(stddev=0.0))
